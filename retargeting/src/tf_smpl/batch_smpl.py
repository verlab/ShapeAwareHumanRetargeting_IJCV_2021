...
class SMPL(object):
    def __init__(self, pkl_path, joint_type='cocoplus', dtype=tf.float32):
        ...
        with open(SMPL_MODEL_PATH, 'rb') as f:
            ddd = pickle.load(f,encoding='latin1')  

        self.joint_regressor = tf.Variable(
            ddd['cocoplus_regressor'].T.todense(),
    ...
    def __call__(self, beta, theta, get_skin=False, name=None,get_A_global=False):
        """
        ...
        with tf.name_scope(name, "smpl_main", [beta, theta]):
            ...
            if get_A_global:
                self.J_transformed, A,A_global = batch_global_rigid_transformation(Rs, J, self.parents,get_A_global=get_A_global)
