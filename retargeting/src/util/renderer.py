...
class SMPLRenderer(object):
    def __init__(self,
                 ...
                 near=0.1,
                 far=20.0,
                 face_path="tf_smpl/dense_map.npy"):

        self.textura = np.load(os.getcwd() + "/src/tf_smpl/dense_texture.npy")
        ALP_UV = scipy.io.loadmat(os.getcwd() + "/src/tf_smpl/UV_Processed.mat")
        #print (ALP_UV.shape)
        ...
        map_vertices = lambda x:self.All_vertices[x] - 1        
        self.faces = map_vertices(self.FacesDensePose)
        #print (self.faces.shape)
        self.w = frame_width
        self.h = frame_height
        self.near = near
        self.far = far
        self.right = ((self.w/2.0)/flength)*near
    ...
    def dump_textura(self,verts,faces):
        textura = np.zeros((verts.shape[0],3))
        for i in range(self.faces.shape[0]):
           textura[faces[i,0],0] = self.V_norm[self.FacesDensePose[i,0]]
           textura[faces[i,1],0] = self.V_norm[self.FacesDensePose[i,1]]
           textura[faces[i,2],0] = self.V_norm[self.FacesDensePose[i,2]]
           textura[faces[i,0],1] = self.U_norm[self.FacesDensePose[i,0]]
           textura[faces[i,1],1] = self.U_norm[self.FacesDensePose[i,1]]
           textura[faces[i,2],1] = self.U_norm[self.FacesDensePose[i,2]]
           textura[faces[i,0],2] = self.FaceIndices[i]/24.0
           textura[faces[i,1],2] = self.FaceIndices[i]/24.0
           textura[faces[i,2],2] = self.FaceIndices[i]/24.0

        np.save("dense_texture.npy",textura)
...
def render_model():
    ...
    pixels = dirt.rasterise(
        ...
        background=tf.ones([frame_height, frame_width, 3]),
        width=frame_width, height=frame_height, channels=3
    )
    return pixels
    ...
    #    cv2.imwrite('simple.jpg', pixels_eval[:, :, (2, 1, 0)]*255.0)
        #print (pixels_eval[0,0,1])
        #cv2.waitKey(0)
...
def get_original(proc_param, verts, cam, joints, img_size):
    ...
    kp_original = np.zeros((19,2))#(joints + proc_param['start_pt'] - margin) * undo_scale
